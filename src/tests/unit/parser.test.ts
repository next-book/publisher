...
describe('forbidden input', () => {
  ...
  it('should throw when undefined delimiter provided', () => {
    ...
    expect(() =>
      parse(document.querySelector('main')!, undefined as unknown as string)
    ).toThrowError('Expected delimiter undefined.');
  });
});
...
describe('basic whitespace input', () => {
  ...
  it('should return ParsedObj with whitespace idea', () => {
    const rawmain = document.createElement('main');
    rawmain.appendChild(document.createTextNode('   '));
    rawmain.appendChild(document.createTextNode('\n'));
    rawmain.appendChild(document.createTextNode(' '));
    rawmain.appendChild(document.createTextNode('\n'));
    rawmain.appendChild(document.createTextNode(' content'));
    document.body.appendChild(rawmain);
    const parsed = parse(document.querySelector('main')!, '\n');
      ...
      new ParsedObj(
        ...
        [
          [
            new ParsedObj(
              paragraph,
              [[new ParsedObj(italic, [['para'], ['graph 1']], '\n')]],
              '\n'
            ),
          ],
        ],
